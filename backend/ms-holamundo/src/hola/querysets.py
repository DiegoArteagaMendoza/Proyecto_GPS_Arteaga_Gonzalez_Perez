...
class HolaMundoQuerySet(models.QuerySet):
    @staticmethod
    def consulta_prueba():
        Prueba = apps.get_model('hola', 'Prueba')#el de la derecha el nombre del modelo el de la izquierda el nombre aplicacion
        respuesta = Prueba.objects.all()
        serializer = PruebaSerializer(respuesta, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    
    def crear_prueba( request):
        serializer = PruebaSerializer(data=request.data)#recupera info del request y la manda
